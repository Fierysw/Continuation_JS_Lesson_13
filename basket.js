...
async function fetchData() {
  try {
      ...
      data.forEach(({ id, name, image, price, text }) => {
          const productEl = `
          <div class="featuredItem" data-id="${id}" data-name="${name}" data-price=${price}>
...
function renderNewProductInBasket(productId) {
  const productRow = `
    <div class="basketRow" data-id="${productId}">
      ...
      <div>
      <button class="btnDelite">Удалить</button>
      </div>
    </div>
    `;

  basketTotalEl.insertAdjacentHTML("beforebegin", productRow);
  const deleteBtn = document.querySelectorAll('.btnDelite')
      deleteBtn.forEach((button) => {
          button.addEventListener('click', () => {
              const product = button.closest('.basketRow');
              product.remove();
          })
      })
}
